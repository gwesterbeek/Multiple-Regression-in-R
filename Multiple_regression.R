...
FinalData<- filter(FinalData1,PositiveServiceReview < 400 & 
              x4StarReviews <400 & x2StarReviews <300 & Volume != 1576 & Volume != 2140)
...
train_results
### Results on test 
test_results <- predict(knnFit1, newdata = testing)  
postResample(testing$Volume, test_results)
...
### Results on train
train_results <- predict(svmFit1, newdata = training)  
postResample(training$Volume, train_results)
...
# Results on test ####
test_results <- predict(svmFit1, newdata = testing)  
postResample(testing$Volume, test_results)
